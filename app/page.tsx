...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-[#f5f6f8]">
      ...
      {/* Nav */}
      <header className="sticky top-0 z-20 border-b border-[#e7e9ee] bg-white/85 backdrop-blur-md">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3">
          ...
          <Link href="/console" className="flex items-center gap-2 rounded-lg bg-gradient-to-r from-indigo-500 to-violet-500 px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:opacity-90">
        ...
        <div className="mx-auto max-w-7xl px-6 pt-16 pb-12">
          <div className="animate-fade-up max-w-3xl">
            ...
            <p className="mt-4 max-w-2xl text-base leading-relaxed text-gray-600">
              ...
              Guided Transport Systems — Metro, Tram, Automated People Movers and Autonomous Vehicles.
              Map evidence to certification requirements, monitor operator Safety Management Systems, and
              keep regulatory alignment with global best practice.
            </p>
            <div className="mt-7 flex flex-wrap items-center gap-3">
              <Link href="/console" className="flex items-center gap-2 rounded-lg bg-gradient-to-r from-indigo-500 to-violet-500 px-5 py-2.5 text-sm font-medium text-white shadow-sm transition hover:opacity-90">
                Open Monitoring Console <ArrowRight className="h-4 w-4" />
              </Link>
              <a href="#capabilities" className="rounded-lg border border-[#e7e9ee] bg-white px-5 py-2.5 text-sm font-medium text-gray-700 transition hover:border-indigo-300">
                Explore Capabilities
              </a>
            </div>
            <div className="mt-6 flex flex-wrap gap-2">
              {TERMS.map((t) => (
                <span key={t} className="rounded-full border border-[#e7e9ee] bg-white/70 px-3 py-1 text-xs text-gray-600">{t}</span>
              ))}
            </div>
          </div>
          ...
          <div className="mt-12 grid grid-cols-2 gap-4 lg:grid-cols-4">
            ...
            <Stat value={String(STANDARDS.length)} label="Safety Standards Monitored" />
          ...
          {PILLARS.map((p, i) => {
            ...
            return (
              <div key={p.title} className="card card-hover p-5">
                <div className="grid h-11 w-11 place-items-center rounded-xl bg-gradient-to-br from-indigo-500/15 to-violet-500/15 text-indigo-600">
                  <Icon className="h-5 w-5" />
                </div>
                <p className="mt-3 text-sm font-semibold text-gray-900">{p.title}</p>
                <ul className="mt-2 space-y-1">
                  {p.items.map((it) => (
                    <li key={it} className="flex items-center gap-1.5 text-xs text-gray-500">
                      <CheckCircle2 className="h-3 w-3 text-emerald-500" /> {it}
                    </li>
                  ))}
                </ul>
              </div>
      ...
      {/* Capabilities */}
      <section id="capabilities" className="bg-white py-12">
            ...
            {CAPABILITIES.map((c, i) => {
              ...
              return (
                <div key={c.name} className="card p-5">
                  <div className="flex items-center gap-3">
                    <div className="grid h-10 w-10 place-items-center rounded-lg bg-gradient-to-br from-indigo-500 to-violet-500 text-white"><Icon className="h-5 w-5" /></div>
                    <div>
                      <p className="text-sm font-semibold text-gray-900">{c.name}</p>
                      <p className="text-[11px] font-medium text-indigo-600">{c.need}</p>
                    </div>
                  </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-12">
        ...
        <div className="mt-5 flex flex-wrap gap-2.5">
          {STANDARDS.map((s) => (
            <div key={s.id} className="flex items-center gap-2 rounded-full border border-[#e7e9ee] bg-white px-3.5 py-2 text-sm">
        ...
        <div className="mt-10 flex flex-col items-start justify-between gap-4 rounded-2xl bg-gradient-to-r from-indigo-500 to-violet-500 px-8 py-7 text-white sm:flex-row sm:items-center">
          <div>
            <p className="text-lg font-semibold">Ready to see continuous compliance in action?</p>
            <p className="text-sm text-white/80">Open the live monitoring console for certification readiness, findings and evidence intake.</p>
          </div>
          <Link href="/console" className="flex items-center gap-2 rounded-lg bg-white px-5 py-2.5 text-sm font-semibold text-indigo-700 transition hover:bg-white/90">
            Open Console <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
      </section>
